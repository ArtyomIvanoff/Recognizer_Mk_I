...
public class OCR extends JFrame {
	...
	class SymAction implements java.awt.event.ActionListener {
		public void actionPerformed(final java.awt.event.ActionEvent event) {
			...
			} else if (object == OCR.this.clear || object == OCR.this.clearText ) {
			...
			} else if (object == OCR.this.recognText) {
                recognText_actionPerformed(event);
            }
		}
	}
    ...
    /**
     * The entry component for the user to draw text into.
     */
    ...
    private final JButton clearText = new JButton();
	/**
	 * The recognize button for letter
	 */
	private final JButton recognize = new JButton();

    //The recognize button for text
    private final JButton recognText = new JButton();
	...
	OCR() {
        ...
        this.entryText = new EntryText();
        this.entryText.setLocation(388, 25);
        this.entryText.setSize(200, 128);
        getContentPane().add(this.entryText);

		this.sample = new Sample(OCR.DOWNSAMPLE_WIDTH, OCR.DOWNSAMPLE_HEIGHT);
		this.sample.setLocation(210, 210);
		this.sample.setSize(65, 70);

		this.entry.setSample(this.sample);
        this.entryText.setSample(this.sample);
        ...
        this.clearText.setText("Clear text");
        this.clearText.setActionCommand("Clear text");
        getContentPane().add(this.clearText);
        this.clearText.setBounds(388, 180, 120, 24);
        ...
        this.recognText.setText("Recognize text");
        this.recognText.setActionCommand("Recognize text");
        getContentPane().add(this.recognText);
        this.recognText.setBounds(388, 156, 120, 24);
		this.JScrollPane1
        ...
        this.JLabel4.setText("or draw Text Here");
        getContentPane().add(this.JLabel4);
        this.JLabel4.setBounds(420, 12, 144, 12);
        ...
        this.clearText.addActionListener(lSymAction);
        ...
        this.recognText.addActionListener(lSymAction);
    ...
    /** Переделай этот метод!
     * Called when the recognText button is pressed.
     *
     * @param event
     *            The event.
     */
    void recognText_actionPerformed(final java.awt.event.ActionEvent event) {
        if (this.net == null) {
            JOptionPane.showMessageDialog(this, "I need to be trained first!",
                    "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        this.entryText.downSample();
        int countChar = this.entryText.dataText.size();
        if(countChar < 1) {
            JOptionPane.showMessageDialog(this, "There is no text!",
                    "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        for(int i = 0; i < countChar; i++){
            input = new BasicMLData(OCR.DOWNSAMPLE_WIDTH * OCR.DOWNSAMPLE_HEIGHT);
            int idx = 0;
            ds = this.entryText.dataText.get(i);
        ...
        JOptionPane
                .showMessageDialog(this, ans, "Text is here",
                        JOptionPane.PLAIN_MESSAGE);
        clear_actionPerformed(null);
